...
| Aha+center+floor | 50.37 @25/30 | 87.96 @35-50 | judge | **不叠加**: 双双低于单项最优 |
...
**TZR (三区重建) 终审 (2026-09-13)**: TB 峰 49.14@30 / V* 峰 90.05@51 — 与 center 完全
同分, 明显低于裸 pair。验尸 (四个设计误判, 详见 vlm_opsa_aha_plan.md 附录):
① 根本性 — 用"错误在哪"的选择思维替代"信号在哪"的蒸馏思维: 三区仅覆盖 2.2% 维度,
98% 维 β=0, loss 腰斩 (0.06 vs pair 0.10), pair 的收益本就来自全支持集微小共识锐化的
累积而非定点压制; ② 上/下行核算不对称 (press 上行趋零 vs 冻结下行 −3 分级);
③ "最小干预"用错对象 (蒸馏里移动学生是目的, 为省 1.7% 回归税砍掉主信号);
④ 给不在场的挤压危险升级盔甲。轨迹形态 = "半速 pair" (TB@51≈pair@20, V* 仍上行),
证实欠训诊断。修正方向 (未跑): zones 叠加于均匀 β 之上而非替代
(β_base 全维保底 + Δβ·press), 按构造不劣于裸 pair。**裸 pair 封为主配方。**

**预算轴收口 (floor ep2, step 55-102, 2026-09-12)**: TB 46.7-50.1 震荡终 50.12 —
**未超 ep1 峰 51.60@50, TB 一个 epoch 内饱和**; V* 持续高位 89-91.62 (峰 91.62@80,
较 ep1 +2.1)。结论: 扩 epoch 非杠杆 (TB 顶已到), view 构造才是; 数据扩充为唯一
未测 scaling 轴; 后续臂 (TZR 等) 用 1 epoch 标准预算。
